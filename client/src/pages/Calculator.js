...
const Calculator = () => {
  ...
  // Ensure valuation is calculated when report is shown
  useEffect(() => {
    if (showReport && (valuationRange.min === 0 && valuationRange.max === 0)) {
  ...
  const nextStep = () => {
    if (validateStep(currentStep)) {
      if (currentStep < 8) {
        ...
        // Trigger valuation calculation when moving from step 7 to step 8
        if (currentStep === 7) {
          // Calculate valuation immediately
          const calculatedRange = calculateValuationRange();
          setValuationRange(calculatedRange);
          setFormData(prev => ({
            ...prev,
            valuation: `${calculatedRange.min.toLocaleString()} - ${calculatedRange.max.toLocaleString()}`
          }));
        }
      } else if (currentStep === 8) {
        // When on contact form, submit and show results immediately
  ...
  const handleFormSubmission = async () => {
    // Ensure valuation is calculated before submission
    if (valuationRange.min === 0 && valuationRange.max === 0) {
      const calculatedRange = calculateValuationRange();
      setValuationRange(calculatedRange);
    }
    ...
    setShowReport(false);
    setLoadingStage("Submitting your request...");
    setLoadingProgress(50);
    
    // Brief delay of 2-3 seconds instead of 20 seconds
    setTimeout(() => {
      const handleSubmission = async () => {
        try {
          ...
          // Immediately show results after successful submission
      ...
      handleSubmission().catch(error => {
        console.error('Unhandled promise rejection in form submission:', error);
        // Ensure UI state is always updated
        setIsCalculating(false);
        setShowReport(true);
        setCurrentStep(9);
      });
    }, 2500); // Reduced from 20 seconds to 2.5 seconds
  };

  // Removed startLoadingProcess function - no longer needed with immediate results
  ...
  const calculateValuationRange = () => {
    ...
    // Ensure minimum valuation is never 0
    const finalBaseValuation = Math.max(baseValuationFinal, 10000);
    
    // Calculate valuation range (±15% variance)
    const variance = 0.15;
    const minValuation = Math.round(finalBaseValuation * (1 - variance));
    const maxValuation = Math.round(finalBaseValuation * (1 + variance));
  ...
  const submitToFormspree = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/contact/submit', {
        ...
        body: JSON.stringify({
          firstName: formData.companyName.split(' ')[0] || 'User',
          lastName: formData.companyName.split(' ').slice(1).join(' ') || 'Name',
          email: formData.email,
          phone: formData.phone || '',
          company: formData.companyName,
          subject: `Business Valuation Request - ${formData.companyName}`,
          message: `BUSINESS VALUATION CALCULATOR SUBMISSION
...
Valuation Calculator User`,
          serviceInterest: 'Business Valuation',
          formType: 'Business Valuation Calculator'
        })
      });
...
Valuation Calculator User
  ...
  return (
      ...
      case 7:
        ...
        case 8.5: // Loading state
          return (
            <LoadingContainer>
              ...
              <LoadingSubtext>Please wait while we analyze your business data... This may take up to 3 seconds.</LoadingSubtext>
        ...
        case 9:
        // Ensure valuation is calculated if not already done
        const currentValuationRange = valuationRange.min === 0 && valuationRange.max === 0 
            ...
            <ValuationDisplay>
              ...
              <ValuationRange>
                <ValuationAmount>
                  ...
                  {currentValuationRange.min.toLocaleString()} – {currentValuationRange.max.toLocaleString()}
                </ValuationAmount>
              </ValuationRange>
            </ValuationDisplay>
