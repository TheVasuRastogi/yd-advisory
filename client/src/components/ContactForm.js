...
const ContactForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      console.log('Contact form submission started with data:', data);
      
      // Try EmailJS first
      const emailjsResult = await emailjsService.sendContactForm({
        ...data,
      ...
      if (emailjsResult.success) {
        ...
        console.log('EmailJS failed, trying backend fallback...');
      ...
      // Final fallback to Web3Forms
      try {
        console.log('Backend failed, trying Web3Forms fallback...');
        const formData = new FormData();
        formData.append("access_key", "4d97b7da-7130-49e4-897a-50bd925206fe");
        ...
        if (result.success) {
          ...
          setSubmitMessage('Failed to send message. Please try again or contact us directly at Yashaswi.das@ydadvisory.ae');
        }
      } catch (fallbackError) {
        console.error('All email methods failed:', fallbackError);
        setSubmitStatus('error');
        setSubmitMessage('Failed to send message. Please contact us directly at Yashaswi.das@ydadvisory.ae or call +971-528477349');
      }
    } finally {
